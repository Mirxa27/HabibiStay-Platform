-- Create Admin User for Testing HabibiStay Application
-- This script creates a comprehensive admin user with full access
...
-- If permissions table exists (from migration 10), grant admin permissions
INSERT OR IGNORE INTO user_permissions (user_id, permission_id, granted_at, granted_by)
SELECT 'admin-test-12345', p.id, CURRENT_TIMESTAMP, 'system'
FROM permissions p
WHERE p.category IN ('admin', 'properties', 'bookings', 'users', 'payments');

INSERT OR IGNORE INTO user_permissions (user_id, permission_id, granted_at, granted_by)
SELECT 'admin-owner-67890', p.id, CURRENT_TIMESTAMP, 'system'
FROM permissions p
WHERE p.category IN ('admin', 'properties', 'bookings', 'users', 'payments');
